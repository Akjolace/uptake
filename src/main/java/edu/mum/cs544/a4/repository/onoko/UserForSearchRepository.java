...
public interface UserForSearchRepository extends JpaRepository<UserForSearch, Long>{
    ...
    @Query(value="select u.username, p.bio, i.path from user u inner join profile p on u.id = p.user_id left join photo i on p.photo_id = i.id  where u.username like %"+ ":username" +"% limit 10;", nativeQuery = true)
